...
public class ProfilRep {
	...
	public ProfilRep(PackageInfo p, Model m, Options o,
			ShapeChangeResult r, String ma, String n, boolean g, String v) {
		pi = p;
		model = m;
		options = o;
		result = r;
	...
	public ProfilRep(PackageInfo p, Model m, Options o,
			ShapeChangeResult r, String ma, String n, String v) {
		pi = p;
		model = m;
		options = o;
		result = r;
	...
	private void addToModel(Info i) {
		String s1 = i.taggedValue(tag());
		if (s1==null || s1.trim().length()==0) {
			// only add to AAA:Profile, for AAA:Modellart a blank value means all Modellarten are included
			if (!setModellartOnly()) {
		...
		if (export) {
			...
			if (!codedValue) {
				...
				if (!propi.isAttribute() && propi.isNavigable()) {
					// Bei einer Relation sind auch alle inversen Rollen automatisch Teil des Profils
					PropertyInfo rpropi = propi.reverseProperty();
					if (rpropi!=null) {
						Members.add(rpropi);
					}
	...
	public void writeToModel() {
		if (!set) {
			result.addError("Übernahme des Profils oder der Modellart in das Modell nicht möglich, da keine Definition vorliegt.");
			return;
		}
	...
	public ProfilRep(PackageInfo p, Model m, Options o,
			ShapeChangeResult r, String v, String filename) {
		pi = p;
		model = m;
		options = o;
		result = r;
		...
		try {
			...
			try {
				...
				line = input.readLine();
				if (line==null || !(line.contains("AFIS-ALKIS-ATKIS-Profildefinition") || line.contains("AFIS-ALKIS-ATKIS-Modellartdefinition"))) {
					result.addError("Die Profildatei beginnt nicht mit dem erwateten Wert, 'AFIS-ALKIS-ATKIS-Profildefinition' oder 'AFIS-ALKIS-ATKIS-Modellartdefinition'. Der Ladevorgang wurde abgebrochen.");
					error = true;
					return;
				}
				...
				if (line==null || !line.trim().startsWith("Version: ")) {
					...
					if (!s.equals(aaaZielVersion)) {
						result.addError("Die Version "+s+" des Profils passt nicht zur erwarteten Version "+aaaZielVersion+". Der Ladevorgang wurde abgebrochen.");
						error = true;
						return;						
					}
				}
							...
							if (s[1].equals("Objektart/Datentyp")) {
								...
								if (cix!=null && cix.inSchema(pi)) {
									if (MatchingMA(cix,false) || setModellartOnly()) {
										...
										result.addError("Objektart/Datentyp "+s[0]+" kann dem Profil nicht zugeordnet werden, da es kein Bestandteil der betreffenden Modellart ist.");
									}
								} else {
									result.addError("Objektart/Datentyp "+s[0]+" kann dem Profil oder der Modellart nicht zugeordnet werden, die Klasse wurde nicht im Anwendungsschema gefunden.");
								}
								...
								if (s2.length!=2) {
									...
									if (cix!=null && cix.inSchema(pi)) {
										if (MatchingMA(cix,false) || setModellartOnly()) {
											...
											if (!found) {
												result.addError(s[1]+" "+s2[1]+" zu Objektart/Datentyp "+s2[0]+" kann dem Profil nicht zugeordnet werden, die Eigenschaft wurde nicht im Anwendungsschema gefunden.");
											}
										} else {
											result.addError(s[1]+" "+s2[1]+" zu Objektart/Datentyp "+s2[0]+" kann dem Profil nicht zugeordnet werden, da sie kein Bestandteil der betreffenden Modellart ist.");
										}
									} else {
										result.addError("Objektart/Datentyp "+s2[0]+" kann dem Profil nicht zugeordnet werden, die Klasse wurde nicht im Anwendungsschema gefunden.");
									}
								}
							}
						}						

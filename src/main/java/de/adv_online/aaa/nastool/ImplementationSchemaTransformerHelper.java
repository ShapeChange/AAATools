...
public class ImplementationSchemaTransformerHelper {
    ...
    public HashMap<String, Element> aaaClasses = new HashMap<String, Element>();
    private int SeqNo = 32000;
    protected String aaaVersion = "unbekannt";
//    private String path = null;
//    protected Package aaaPackage = null;

    public void initialise(Options o, ShapeChangeResult r, String repositoryFileName)
...
//	path = o.parameter("transformerTargetPath");
     ...
     * Eindeutigkeit der Ei- genschaftsnamen zu gewährleisten. - Die
     * <<Type>>-Klassen werden gelöscht.
     * 
     ...
     * “true” bei <<FeatureType>>; “false” bei <<DataType>> und <<Union>> -
	...
	for (Element e : aaaClasses.values()) {
	    ...
	    if ((st.equals("enumeration") || type.equals("enumeration")) && (n.equals("AX_BezugspunktDach")
	...
	for (AttributeTag tv : a.GetTaggedValues()) {
	    ...
	    if (!tv2.Update()) {
		result.addError("Fehler beim Clonen von Attribut '" + s + "' (Zielklasse '" + s2
			+ "') beim Kopieren von Tagged Value '" + tv.GetName() + "': " + tv.GetLastError());
	    }
	...
	if (tv == null) {
		...
		tv.SetValue(v);
		if (!tv.Update()) {
		    result.addError("Fehler beim Setzen von TaggedValue '" + n + "'-'" + v + "'/'" + v2 + "': "
			    + tv.GetLastError());
		} else {
	...
	if (tv == null) {
		...
		result.addDebug("Setzen von TaggedValue '" + n + "'-'" + v2
			+ "' (alter Wert wird nicht geändert durch '" + v + "')");
	    } else {
		tv.SetValue(v);
		if (!tv.Update()) {
		    result.addError("Fehler beim Setzen von TaggedValue '" + n + "'-'" + v + "'/'" + v2 + "': "
    ...
    /*
     * unused private void createClass(String name, String pname) {
     * org.sparx.Package p = allPackages.get(pname); if (p!=null) {
     * org.sparx.Element e = p.GetElements().AddNew(name, "Class"); if (!e.Update())
     * {
     * result.addError("Fehler beim Erzeugen von Klasse '"+name+"': "+e.GetLastError
     * ()); } else { result.addDebug("Klasse '"+name+"' erzeugt."); } } else {
     * result.addError("Package '"+pname+"' nicht gefunden."); } }
     * 
     * private void moveClass(String name, String pname) { org.sparx.Element e =
     * allClasses.get(name); if (e!=null) { org.sparx.Package p =
     * allPackages.get(pname); if (p!=null) { e.SetPackageID(p.GetPackageID()); if
     * (!e.Update()) {
     * result.addError("Fehler beim Verschieben von Klasse '"+name+"': "+e.
     * GetLastError()); } else { result.addDebug("Klasse '"+name+"' verschoben."); }
     * } else { result.addError("Package '"+pname+"' nicht gefunden."); } } else {
     * result.addError("Klasse '"+name+"' nicht gefunden."); } }
     */
    ...
    public void prepareModel(SortedMap<String, String> schemaMappings) throws ShapeChangeAbortException {
	for (Package p : rep.GetModels()) {
	    preparePackage(p, schemaMappings, false);
	}
    }

    public void prepareModel(String appSchemaName, String implSchemaName) throws ShapeChangeAbortException {
	for (Package p : rep.GetModels()) {
	    preparePackage(p, appSchemaName, implSchemaName, false);
	}
    }

    private void preparePackage(Package p, String appSchemaName, String implSchemaName, boolean aaa)
	    throws ShapeChangeAbortException {
	...
	if (p.GetName().equals(appSchemaName)) {
	    ...
	    Package aaaPackage = p.Clone();
	    p = aaaPackage;
	    if (p == null) {
		result.addFatalError(
			"Fehler beim Klonen des Anwendungsschemas. Ggf. ist das AdV-Paket noch mit dem SVN verbunden.");
		throw new ShapeChangeAbortException();
	    } else {
	...
	commonPackagePreparations(p, aaa);
    }

    private void preparePackage(Package p, SortedMap<String, String> schemaMappings, boolean aaa)
	    throws ShapeChangeAbortException {

	String originalPackageName = p.GetName();

	if (schemaMappings.containsKey(originalPackageName)) {

	    String appSchemaName = originalPackageName;
	    String implSchemaName = schemaMappings.get(appSchemaName);

	    aaa = true;

	    result.addInfo("Anwendungsschema " + appSchemaName
		    + " wird im Repository verändert jedoch NICHT geklont! Der Name wird geändert zu: "
		    + implSchemaName);
	...
	for (org.sparx.Package p2 : p.GetPackages()) {
	    preparePackage(p2, schemaMappings, aaa);
	}

	commonPackagePreparations(p, aaa);
    }

    private void commonPackagePreparations(Package p, boolean aaa) {
...
//	    }
	...
	for (Element e : c) {
		...
		if (aaa) {
			...
			result.addError(
				"Information: Classifier '" + s + "' im Implementierungsschema mehrfach vorhanden.");
		    } else {
			aaaClasses.put(s, e);
		    }
		}
